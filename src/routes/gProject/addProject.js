...
export default class AddProject   extends PureComponent {
  ...
  render() {
    ...
    const { submitting,form,dispatch,progroupdata,prolinedata } = this.props;
         ...
         <FormItem
          ...
          >
            {getFieldDecorator('selectline')(
              ...
              >
                { prolinedata.length > 0 && prolinedata.map(post => {
          ...
          >
            {getFieldDecorator('selectgroup')(
              <Select
                ...
                filterOption={(input, option) => option.props.children.toLowerCase().indexOf(input.toLowerCase()) >= 0}
              >
                { progroupdata.length > 0 && progroupdata.map(post =>
