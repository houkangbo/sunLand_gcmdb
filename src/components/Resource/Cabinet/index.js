...
class CabTable extends PureComponent {
    ...
    columns = [{
    ...
    },
     {
        ...
        let columnsNextTab=[{
            dataIndex: 'bay_name',
            width: '20%',
            render: (text, childRecord) => this.renderChangeTab(text, childRecord, parentRecord, 'bay_name')
          },{
            dataIndex: 'height',
            width: '15%',
            render: (text, childRecord) => this.renderChangeSel(text, childRecord, parentRecord, 'height')
          },{
            dataIndex: 'status',
            width: '20%',
            render: (value, childRecord) => {
              return(<Switch 
                ...
                onChange={(value)=>{this.swithChange(value,childRecord, parentRecord,"status")}}
              />)
            }
          }]
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.cabinet.data) {
        this.setState({
            ...
            cabinetdata: nextProps.cabinet.data.cabinetdata == undefined ? [] : nextProps.cabinet.data.cabinetdata.map((obj,inx)=>{
              obj.tabStatus = false
              return obj
            })
        })
  ...
  renderChangeSel(text, childRecord, parentRecord, tabNodeName){
    ...
    let optionEL=bay_heights.map((obj)=>{
      return (<Option label="bay_heights" key={obj.ID} value={obj.height}>{obj.height}</Option>)
    })
    return (
      <Select 
        size="small"
        ...
        onChange={(value)=>{this.slelctChangeTab(value,childRecord, parentRecord,tabNodeName)}}
      >    
        {optionEL}
      </Select>
    )
  ...
  renderChangeTab(text, childRecord, parentRecord, tabNodeName){
    ...
    (<div className="editable-cell-input-wrapper">
      <Input
        size="small"
        value={text}
        onChange={(e)=>{this.handleChangeTab(e,childRecord, parentRecord,tabNodeName)}}
      />
    </div>) 
      : 
  ...
  render() {
    ...
    this.cacheData = data.cabinetdata == undefined ? [] : data.cabinetdata.map(item => ({ ...item }));                
    return  (
